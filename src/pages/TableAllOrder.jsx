...
import moment from "moment";
  ...
  const TableAllOrder = ({ orderData }) => {
    ...
    const showModalUserOrder = async (user) => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}user/${user}`);
        ...
        const data = {
          ...
          status:
            item.orderStatus === "Đã xác nhận" ? (
              ...
              <span className="text-[#27ae60] font-bold">Đơn hàng đã được giao</span>
  ...
  {userData && (
    <tr>
      ...
      <td className="px-6 py-3">{userData.firstname} {" "} {userData.lastname}</td>
